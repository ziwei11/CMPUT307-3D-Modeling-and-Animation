...
def searchCodeWord(root, symbol):
    if root == None:      # if no root
        return None
    if root.getLeft() == None and root.getRight() == None:      # if leaf node
...
def searchSymbol(huffman_tree, code_word):
    if huffman_tree == None:      # if no root
        return None
    if huffman_tree.getLeft() == None and huffman_tree.getRight() == None:      # if leaf node
...
def assignCodeWord(root, code_word=''):
    ...
    if root.getSymbol() == None:        # if the root is internal node 
